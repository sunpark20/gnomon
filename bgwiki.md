...
Gnomon(DDC)과 Century Iris(감마) 모두 동일한 철학을 공유한다: **모니터 밝기 ≈ 옆에 놓은 A4 용지 밝기**일 때 눈이 가장 편하다.

### DDC 시절 (Gnomon)

DDC 0~100 범위에서 low floor(최저 조도 환경)의 앵커를 잡고 로그 곡선을 생성.

### 감마 시절 (Century Iris)

감마 0.08~1.0 범위에서 같은 방법을 적용하되, 완전 검정(0.0)까지 내리지 않는다. DDC 최저 밝기와 비슷한 수준을 감마 최저값으로 설정한다.

**캘리브레이션 절차:**

1. 어두운 환경(low lux)을 만든다
2. 감마를 DDC 최저 밝기 수준까지 내린다 (완전 검정 아님)
3. A4 용지를 모니터 옆에 대본다
4. 감마 슬라이더를 올려서 **화면 밝기 ≈ A4 용지** (또는 약간 밝음) 지점을 찾는다
5. 그 **(lux, 감마값) 쌍**이 앵커 포인트가 된다
6. 이 앵커를 기준으로 위아래 밝기 곡선을 자동 생성

**핵심:** 사용자마다 모니터, 환경이 다르므로 이 한 번의 캘리브레이션이 개인화된 곡선의 기초가 된다.
