...
저는 MacBook Pro에 LG HDR 4K 외장 모니터를 연결해서 씁니다. 문제 하나:
...
DDC(Display Data Channel)는 모니터와 컴퓨터가 통신하는 표준 규약입니다. 이걸로 밝기·대비·입력 소스 같은 걸 프로그램에서 명령으로 바꿀 수 있습니다. Gnomon은 [m1ddc](https://github.com/waydabber/m1ddc)라는 오픈소스 도구를 사용해서 Apple Silicon Mac에서 외장 모니터로 DDC 명령을 보냅니다.
...
대비는 **사용자가 한 번 맞춰둔 값(기본 70%, LG 출하 기본값)을 그대로 유지**합니다. 자동 조정 없습니다.
...
| DDC 전송 | [m1ddc](https://github.com/waydabber/m1ddc) (오픈소스) |
| 곡선 | `b = 20 + 75 × log10(lux+1) / log10(2001)` |
...
- **외장 모니터를 DDC로 제어 가능한 모델 사용** (LG HDR 4K, Dell U-시리즈, Samsung 대부분 등 — 거의 모든 외장 모니터)
...
1. **단일 외장 모니터**: 첫 번째 감지된 모니터만 자동 조정. 멀티 모니터는 v2.
2. **하드웨어 의존**: m1ddc가 인식 못 하는 모니터는 동작 불가 (드물지만 가능)
3. **곡선 파라미터 개인차**: 제 환경에 맞춰 튜닝됐습니다. 본인 모니터 사양/선호에 따라 슬라이더 조정 필요
4. **macOS 15+ 만 지원**: 최신 API 사용. 구버전 지원 계획 없음
5. **코드 서명 안 됨**: macOS Gatekeeper에서 한 번 우회 필요
