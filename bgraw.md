...
## [2026-04-26] 소프트웨어 컨트롤 앱은 별도 프로젝트로 분리 결정

2번(소프트웨어 컨트롤 + 앱스토어 출시)은 앱을 하나 더 만들겠다. 아예 분리.

Gnomon은 DDC 하드웨어 제어 전용으로 유지하고, 소프트웨어 감마 방식의 앱스토어용 앱은 별도 프로젝트로 분리하기로 결정. 이유: DDC(비공개 IOAVService API)와 소프트웨어 감마 제어는 핵심 메커니즘이 완전히 다르고, 앱스토어 샌드박스 요건과 DDC 앱의 자유도를 양쪽 다 살리려면 분리가 합리적.
